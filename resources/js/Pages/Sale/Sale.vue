    ...
    <div>
        <breeze-authenticated-layout>
            ...
            <div class="container-fluid mt-2">
                <form method="POST" @submit.prevent="saveSale">
                    <div class="row">
                        <product-component :from="from" :sale="sale" ref="product" />
                        <div class="col-md-12">
                            <div class="row">
                                <customer-component :sale="sale" :from="from" ref="customer" class="col-md-7"/>
                                <payment-component :sale="sale" :from="from" ref="payment" class="col-md-5"/>
                            </div>
                        </div>
                    </div>
                </form>
...
export default {
    ...
    methods: {
        ...
        confirmSale(){
            ...
            form.customer_type = this.$refs.customer.customerType
            form.sms = this.$refs.customer.sms
            form.note = this.$refs.customer.note
            if (this.$refs.customer.customerId) {
                form.party_id = this.$refs.customer.customerId
            }else{
                form.party_name = this.$refs.customer.customer.name
                form.party_phone = this.$refs.customer.customer.phone
                form.party_address = this.$refs.customer.customer.address
            }
            form.showroom_id = this.$refs.product.showroom_id
            form.date = this.$refs.product.date
            form.payment_type = this.$refs.payment.payment_type
            ...
            form.labour_cost = this.$refs.payment.labourCost || 0
            form.transport_cost = this.$refs.payment.transportCost || 0
            ...
            if (this.$refs.payment.dueOrChange > 0) {
                ...
                form.customer_balance = 0
            }else{
                form.customer_balance = Math.abs(this.$refs.payment.dueOrChange)
                form.due = 0
            }
            ...
            if (this.sale) {
                Inertia.put(this.route("sale.update", this.sale.id), form)
            }else{
                Inertia.post(this.route("sale.store"), form)
            }
