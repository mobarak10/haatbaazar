                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5>{{ lang.expense_report }}</h5>
                            <div>
                                <refresh-icon :Url="route('report.expense-report')" />
                                <search-icon Url="#expense-search-collapse" />
                                <print-icon />
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- search form start -->
                    <div class="card-body">
                        <div class="print-none" :class="[search ? '' : 'collapse']" id="expense-search-collapse">
                        ...
                        <div class="mt-2 row col-md-12">
                            <div class="col-md-12">
                                <table class="table table-bordered table-sm">
                                    ...
                                    <tbody>
                                        <tr v-if="expenseDetails" v-for="(costSubCategory, index) in expenseDetails" :key="index">
                                            <td class="text-wrap">{{ index + 1 }}</td>
                                            <td>{{ costSubCategory.name }}</td>
                                            <td class="text-end" v-for="(month, index) in months" :key="index">
                                                {{ costSubCategory.sum_of_each_month[month] ? Number.parseFloat(costSubCategory.sum_of_each_month[month]).toFixed(2) : Number.parseFloat(0).toFixed(2) }}
                                            </td>
                                            <td class="text-end">
                                                {{ Number.parseFloat(getLineTotal(costSubCategory)).toFixed(2) }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="14" class="text-end">{{ lang.total }}</td>
                                            <td class="text-end">
                                                {{ Number.parseFloat(getGrandTotal(expenseDetails)).toFixed(2) }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
...
export default {
    ...
    methods: {
        getYearWiseData() {
            ...
            this.search = true;
            this.$inertia.visit('/report/expense-report', {
                method: 'get',
                data: {
                    search: true,
